...
public section.
  ...
  methods ADDRESS
    importing
      !ROW_ABSOLUTE type ABAP_BOOL default ABAP_TRUE
  ...
  methods CELLS
    ...
    returning
      value(RESULT) type ref to ZCL_XLOM_RANGE .
  methods COLUMNS
    returning
      value(RESULT) type ref to ZCL_XLOM_RANGE .
  methods COUNT
  ...
  class-methods CREATE
    importing
      !CELL1 type ref to ZCL_XLOM_RANGE
  ...
  class-methods CREATE_FROM_EXPR_RANGE
    importing
      !EXPR_RANGE type ref to ZCL_XLOM__EX_EL_RANGE
      !RELATIVE_TO type ref to ZCL_XLOM_WORKSHEET
    returning
      value(RESULT) type ref to ZCL_XLOM_RANGE
    raising
...
CLASS ZCL_XLOM_RANGE IMPLEMENTATION.
  ...
  METHOD address.
    " TODO: parameter ROW_ABSOLUTE is never used (ABAP cleaner)
    " TODO: parameter COLUMN_ABSOLUTE is never used (ABAP cleaner)
...
*    DATA(structured_address) = VALUE ZIF_xlom_result_array=>ts_address(
...
*    DATA(address) = VALUE ZIF_xlom_result_array=>ts_address(
...
*    IF relative_to->parent->application->reference_style = ZCL_xlom=>c_reference_style-a1.
...
*    ELSE.
*      RAISE EXCEPTION TYPE ZCX_xlom_todo.
**      result = ZCL_xlom_range=>decode_range_address_r1_c1( address ).
*    ENDIF.
...
*        endif.
*        DATA(worksheet_used_range) = CAST ZCL_xlom_range( lookup_array_result )->parent->used_range( ).
