  ...
  METHOD create_expression.
    CASE name.
      WHEN '+'.
        ...
        ENDIF.
        result = zcl_xlom__ex_op_plus=>create( left_operand  = operands[ 1 ]
                                               right_operand = operands[ 2 ] ).
      WHEN '-'.
        IF unary = abap_false.
          result = zcl_xlom__ex_op_minus=>create( left_operand  = operands[ 1 ]
                                                  right_operand = operands[ 2 ] ).
        ELSE.
          result = zcl_xlom__ex_op_minus_unary=>create( operand = operands[ 1 ] ).
        ENDIF.
      WHEN '*'.
        result = zcl_xlom__ex_op_mult=>create( left_operand  = operands[ 1 ]
                                               right_operand = operands[ 2 ] ).
      WHEN '='.
        result = zcl_xlom__ex_op_equal=>create( left_operand  = operands[ 1 ]
                                                right_operand = operands[ 2 ] ).
      WHEN '&'.
        result = zcl_xlom__ex_op_ampersand=>create( left_operand  = operands[ 1 ]
                                                    right_operand = operands[ 2 ] ).
      WHEN ':'.
        result = zcl_xlom__ex_op_colon=>create( left_operand  = operands[ 1 ]
                                                right_operand = operands[ 2 ] ).
      WHEN OTHERS.
        RAISE EXCEPTION TYPE zcx_xlom_todo.
    ENDCASE.
