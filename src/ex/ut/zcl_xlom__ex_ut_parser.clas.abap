...
public section.
  ...
  types:
    tt_expr TYPE STANDARD TABLE OF REF TO zif_xlom__ex WITH EMPTY KEY .

  class-methods CREATE
    returning
      value(RESULT) type ref to ZCL_XLOM__EX_UT_PARSER .
  methods PARSE
    importing
      !TOKENS type ZCL_XLOM__EX_UT_LEXER=>TT_TOKEN
    returning
        ...
        LOOP AT item->subitems INTO subitem.
          ...
          IF subitem->type = zcl_xlom__ex_ut_lexer=>c_type-comma.
            IF    subitem_index = lines( item->subitems )
              ...
              INSERT zcl_xlom__ex_ut_parser_item=>create( type = zcl_xlom__ex_ut_lexer=>c_type-empty_argument ) INTO item->subitems INDEX subitem_index + 1.
            ENDIF.
